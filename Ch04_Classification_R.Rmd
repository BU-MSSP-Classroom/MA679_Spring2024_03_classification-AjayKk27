...
I want to start by definitely looking at pneumonia: Its a major complication of Covid and might be a robust predictor of survivability. Similarly, looking at ICU helps. From the presence of NA's we might assume that while doctors made the decision to put some patients in an ICU or keep them out, others might have just not had the opportunity to be put in an ICU or their condition might have deteriorated very rapidly and suddenly once at the hospital. So let's start with those two:

Your code:
```{r,echo=TRUE}
#
#
fit1<- glm(SURVIVED_bin~PNEUMONIA+ICU +PNEUMONIA*ICU,family=binomial,data= Train_COVID)
summary(fit1)
#Let's evaluate the model:

#Confusion matrix
fit1_predict = predict(fit1,Test_COVID,type="response")
fit1_predict <- ifelse(fit1_predict > 0.5,1,0) # Probability check
CM= table(Covid_Data$SURVIVED_bin, logit_Pred)
print(CM)

#ROC
library(pROC)
roc_score=roc(Test_COVID$SURVIVED_bin, fit1_predict) #AUC score
plot(roc_score ,main ="ROC curve -- Logistic Regression ")

#Well that's pretty abysmal. 
#The fit of this model might as well be random, which is interesting given that superficially I thought this might be a good model. So there might need to be a lot of modifications made to the model. 
```
...
#I know these plots look to be a little backwards in terms of their interpretation in that we're looking at the differences in values of our predictors for values of our outcome, but its an easy way to see if there are substantial differences between the two outcome classes for each predictor.

ggplot(data=auto, aes(mpg01, weight))+geom_boxplot()

ggplot(data=auto, aes(mpg01, horsepower))+geom_boxplot()

ggplot(data=auto, aes(mpg01,year))+geom_boxplot()

ggplot(data=auto, aes(mpg01,displacement))+geom_boxplot()
```
...
#
#
#We'll use a 70-30 split for training vs validation
size<-floor(dim(auto)[1]*0.3)
set.seed(350)
samp_idx<-sample(1:dim(auto)[1],size,FALSE)
auto_Training<- auto[-samp_idx,]
auto_Test <- auto[samp_idx,]
...
#normalizing the variables we will use:
auto_Training_normalized<- auto_Training
auto_Training_normalized[,c(3,5,7)]<- scale(auto_Training_normalized[,c(3,5,7)])
lda_auto<- lda(mpg01~displacement+weight+year, data=auto_Training_normalized)
lda_auto
```

Your answer:
Now we have our lda model, let's test it and evaluate what the test error is:

```{r}
#normalizing Test data first:
auto_Test_normalized<- auto_Test
auto_Test_normalized[,c(3,5,7)]<- scale(auto_Test_normalized[,c(3,5,7)])
lda_predictions<- predict(lda_auto, auto_Test_normalized)

#calculating the error:
mean(lda_predictions$class!= auto_Test_normalized$mpg01)

```
This is a 6% error which I'd say is pretty low. Now, we would probably need to contextualize this error in relation to a null model or the application that we are using this model for to further evaluate whether this is acceptable error or not. 
...
#
#
qda_auto<- qda(mpg01~displacement+weight+year, data=auto_Training_normalized)
qda_auto

#Testing
qda_predictions<- predict(qda_auto, auto_Test_normalized)

#calculating the error:
error<-mean(qda_predictions$class!= auto_Test_normalized$mpg01)
error
```

Your answer:
We have a marginally smaller error than for LDA at 5.9%. Again, this is quite good.
...
logistic_fit<- glm(mpg01~displacement+weight+year, family = 'binomial',data=auto_Training)
summary(logistic_fit)
logistic_probabilities<- predict(logistic_fit, auto_Test,type='response')
logistic_predictions<- rep(1,length(logistic_probabilities))
logistic_predictions[logistic_probabilities<0.5]<-0
error<- mean(logistic_predictions!= auto_Test$mpg01)
error

```

Your answer:
This has the largest error of our three models so far but again, not by much. The error is 6.8% which is really not bad. 
...
#
library(naivebayes)
#Testing that our predictors are not correlated, since the key assumption of naive bayes is that our predictors for each class are independent.

auto_mpg1<- auto_Test |> filter(mpg01==1) |> select('displacement', 'weight','year' )
cor(auto_mpg1)

#Unfortunately weight and displacement are highly correlated. So maybe we'll remove that variable 

naive_bayes_auto<- naive_bayes(mpg01~displacement+year, auto_Training)
plot(naive_bayes_auto)

#Predictions and error:
naive_predictions<- predict(naive_bayes_auto,auto_Test)
#haha ^  
error<- mean(naive_predictions!=auto_Test$mpg01)
error
```

Your answer:
We get an 8.5% prediction error with Naive Bayes. 
~~~
Please write your answer in full sentences.
...
#
library(class)
auto_Training_knn<- auto_Training |> select(-name)
auto_Test_knn<- auto_Test |> select(-name)
#1 nearest neighbor
neighborly1 <- knn(train = auto_Training_knn, 
                      test = auto_Test_knn, 
                      cl = auto_Training$mpg01, 
                      k = 1) 
error1<- mean(neighborly1!= auto_Test$mpg01)

#3 nearest neighbors
neighborly3 <- knn(train = auto_Training_knn, 
                      test = auto_Test_knn, 
                      cl = auto_Training$mpg01, 
                      k = 3) 
error3<- mean(neighborly3!= auto_Test$mpg01)

#10 nearest neighbors
neighborly10 <- knn(train = auto_Training_knn, 
                      test = auto_Test_knn, 
                      cl = auto_Training$mpg01, 
                      k = 10) 
error10<- mean(neighborly10 != auto_Test$mpg01)
...
10 nearest neighbors has the lowest error rate and 1 nearest neighbor has the largest error rate
~~~
Please write your answer in full sentences.
